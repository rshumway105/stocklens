...
CREATE TABLE IF NOT EXISTS signal_history (
    ...
    model_predicted_price   REAL,
    actual_price_at_signal  REAL,
...
def insert_signal_history(
    ...
    model_predicted_price: Optional[float],
    actual_price_at_signal: Optional[float],
) -> None:
    """Record a valuation signal generated by the report builder."""
    conn = _get_db_connection()
    try:
        conn.execute(
            """
            INSERT INTO signal_history
                (ticker, signal, confidence_score, model_predicted_price, actual_price_at_signal)
            VALUES (?, ?, ?, ?, ?)
            """,
            (ticker.upper(), signal, confidence_score, model_predicted_price, actual_price_at_signal),
        )
        conn.commit()
        logger.debug("Recorded signal history for {}: {}", ticker, signal)
    finally:
        conn.close()
...
def get_signal_history(
    ...
    """
    Fetch signal history rows.

    - ticker + days: last `days` days of records for that ticker.
    - latest_per_ticker=True: most recent record per ticker across the watchlist.
    """
    conn = _get_db_connection()
    cols = ["id", "ticker", "signal", "confidence_score",
            "model_predicted_price", "actual_price_at_signal", "created_at"]
    try:
        if latest_per_ticker:
            rows = conn.execute(
                """
                SELECT id, ticker, signal, confidence_score,
                       model_predicted_price, actual_price_at_signal, created_at
                FROM signal_history
                WHERE id IN (
                    SELECT MAX(id) FROM signal_history GROUP BY ticker
                )
                ORDER BY ticker
                """
            ).fetchall()
